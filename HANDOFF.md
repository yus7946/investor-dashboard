...
## 直近の実装履歴（新しい順）
0. **毎時更新＋リスク管理レイヤー＋決算速報**（2026-07-30実装）
   - GitHub Actionsを「朝8:30フル更新＋取引時間中の毎時ライト更新」の2段構成に変更。
     ライト時は環境変数`DASHBOARD_LIGHT=1`で株価系のみ再取得し、EDINET/JPX/ニュース/バックテスト/決算日は
     朝のフル更新が保存した`output/heavy_cache.json`を再利用（IPブロック回避）。
   - `screening/risk.py`新設: 安全度0〜100（ボラ・売り抜け兆候・過熱・空売り・決算接近の減点式）、
     1トレード損失を資金の1〜2%に固定する低リスク運用プラン（条件をすべて満たす銘柄だけのバスケット、
     該当なしの日は「買わないのが規律」と正直表示）。元本から推奨株数を単元100株で逆算しカードに表示。
   - `data/earnings.py`新設: yfinance calendarで決算日取得。「決算速報・決算前警戒」セクション
     （本日発表/5営業日以内、折りたたみ付き）と銘柄カードの決算接近バッジ。
     発表の瞬間の速報は無料では不可能なため「毎時更新での反映」であることをUIに明記済み。
   - 注意: 月20万円目標は「保証」ではなく「損失上限を先に守る」設計にした（ユーザーにもその旨説明済み）。
   - **材料の株価インパクト実績**: 「ニュースがいくら株価に反映されそうか」の要望に対し、予測ではなく
     実測イベントスタディで回答（ふだんの1日σ・出来高2倍以上の日・過去の決算発表翌営業日の騰落を円換算）。
     過去決算日は`data/earnings.py fetch_past_earnings_dates`（yfinance get_earnings_dates、**要lxml**・
     requirements.txtに追加済み）でフル更新時にtop10分のみ取得しキャッシュ。
   - **リサーチメモ**: Perplexity等の外部リサーチを銘柄カードに貼り付け保存（localStorage
     `research_notes_v1`・バックアップ対象・7日超で鮮度警告）。「リサーチ依頼文をコピー」ボタン付き。
   - **機関vs個人投資家トレンドチャート**: JPX投資部門別売買（週次実データ）を`output/flow_history.json`
     に毎回蓄積（同じ週は上書き・最大26週保持、`data/jpx_flow.py update_flow_history`）。
     「海外投資家＋信託銀行」を機関、「個人」を個人投資家として折れ線で推移表示（`renderFlowTrend`）。
     2週未満はデータ蓄積中と正直表示。すべて実測データ・推測は使わない。
1. **機関の資金フロー分析**— ユーザーから「RSIだけのテクニカル分析に見える、機関投資家の実態分析ならCMF/OBV/VWAP/出来高/空売り残高等を組み合わせるべき」と指摘を受け実装。`screening/smart_money.py`で日足OHLCVから追加通信ゼロで計算。スコア最大比重に採用。銘柄カードに「買い集め/売り抜けの兆候」を根拠付きで表示。
